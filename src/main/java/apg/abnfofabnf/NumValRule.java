...
public class NumValRule extends Rule {
    // num-val = "%" (bin-val / dec-val / hex-val)
    private static final Token[] TOKENS = new Token[]{Token.newChar('%'),
            Token.newRuleName("bin-val"), Token.newRuleName("dec-val"),
            Token.newAlternation(), Token.newRuleName("hex-val"),
            Token.newAlternation(), Token.newConcatenation(),};

    public NumValRule() {
        super("num-val", TOKENS);
    }
